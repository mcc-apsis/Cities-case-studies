...
ctab <- ctab %>% 
  rename(pop=Value,country_UN=Country,country_own=country)
...
ctab <- ctab %>%
  select(cities,country_own,country_UN,pop,income,pop_category,income_category,everything())
...
ctab %>% 
  ...
  group_by(pop_category,topic) %>%
  summarise(value=mean(value)) %>%
  
  ggplot(., aes(pop_category,topic)) +
...
ctab %>% 
  gather(topic,value,Active.travel:e.Vehicles) %>%
  group_by(income_category,topic) %>%
  summarise(value=mean(value)) %>%
  
  ggplot(., aes(income_category,topic)) +
  geom_tile(aes(fill = value),colour = "white") +
  scale_fill_gradient(low = "white",high = "steelblue") +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
