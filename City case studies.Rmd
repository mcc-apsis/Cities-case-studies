...
<hr>
#### City case Studies have no distinct trend with city population size, although a few top global cities dominate the analysis
<br>
...
<br>
<hr>
#### Case studies are geographically concentrated in Europe, North America, and Asia
<br>
...
blarg <- blarg %>%
  mutate(Population = geo_norm_pop,Case_studies = study_norm) %>%
  gather(key = "key", value = "value",c(Case_studies,Population))
...
blarg %>%
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
  ylab("Fraction of regional population/case studies per city size") +
  xlab("")
