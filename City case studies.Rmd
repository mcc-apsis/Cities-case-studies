...
ctop %>%
  ...
  scale_x_discrete(labels = c(">10m","5-10m","1-5m","0.5-1m","<0.5m")) +
...
ctop %>%
  mutate(Articles="") %>%
  arrange(-desc(population_category)) %>%
...
ctab <- left_join(ctab,data_income, by = c("geo_city" = "City")) %>%
  rename(gdp_source=gdp,gdp=value) %>%
  mutate(geo_pop=as.numeric(geo_pop))
...
######### group by region and summarise topics #########
blarg <- ctab %>%
  gather(topic,value,Active.travel:e.Vehicles) %>%
  group_by(IAM10,topic) %>%
  summarise(no_studies=n(),no_cities=length(unique(geo_city)))# %>%
...
######### heatmap by region, city size, studies #########

ctab$population_category <- cut(ctab$geo_pop,breaks=c(100e6,10e6,5e6,1e6,5e5,3e5,0),labels=c(">10m","5-10m","1-5m","0.5-1m","0.3-0.5m","<0.3m"),dig.lab=10,include.lowest=TRUE,ordered_result = TRUE)
...
ctab <- ctab %>%
  ...
  summarise(no_studies=n(),no_cities=length(unique(geo_city)),mean_pop=mean(geo_pop)) %>%
  complete(population_category) %>%
  mutate(mean_pop=ifelse(is.na(mean_pop),0,mean_pop))

ctab %>%
  ggplot(.,aes(x=IAM10,y=reorder(population_category,-mean_pop))) +
